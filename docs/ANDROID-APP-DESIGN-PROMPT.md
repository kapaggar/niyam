...
> **Scope: DESIGN ONLY.** Do not write production app code from this file.  
> For implementation (Claude Code → working APK), use  
> [`ANDROID-APP-IMPLEMENTATION-PROMPT.md`](./ANDROID-APP-IMPLEMENTATION-PROMPT.md)  
> after a design doc exists (or use the implementation prompt’s embedded MVP defaults).

**How to use:** Paste this entire document into Claude (or another design agent) as the sole brief for a **system design / product architecture** session. Ask it to produce a design doc, not code.
